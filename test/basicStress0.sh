...
ContainerId=$(curl --data-binary @redis1.json -H "Content-type: application/json" http://127.0.0.1:2379/containers/create | jq -r '.Id')
echo $ContainerId
sleep 1
#Start container
echo "Starting the container..."
#curl -X POST -H "User-token:$UserToken" http://127.0.0.1:3000/containers/$ContainerId/start
curl -X POST -H "User-token:$UserToken" http://127.0.0.1:2379/containers/$ContainerId/start
...
up=$(docker -H 127.0.0.1:2379 ps | wc -l)
